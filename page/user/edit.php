...
if (!isset($_GET['user_id'])) {
    alert_box('alert', get_lang('No ID'));
    exit;
}
?>
<div class="row">
    <div class="four columns">
        ...
        if (isset($_POST['btn_update_user'])) {
            ...
            if ($continue == true) {
                if (update_user(get_the_user('id'), $status, $user_name, $password, $email, $phone, $fax, $mobile, $level)) {
        ...
        <form name="form_login" id="form_login" action="?user_id=<?php the_user('id'); ?>" method="POST">
            <fieldset>
                ...
                <input type="text" name="email" id="email" class="required" minlength="3" maxlength="100" value="<?php the_user('email'); ?>" />

                <label for="phone"><?php lang('Phone'); ?></label>
                <input type="text" name="phone" id="phone" class=""minlength="10" maxlength="11" value="<?php the_user('phone'); ?>" />

                <label for="fax"><?php lang('Fax'); ?></label>
                <input type="text" name="fax" id="fax" class="" minlength="10" maxlength="11" value="<?php the_user('fax'); ?>" />

                <label for="mobile"><?php lang('Mobile'); ?></label>
                <input type="text" name="mobile" id="mobile" class="" minlength="10" maxlength="11" value="<?php the_user('mobile'); ?>" />
                ...
                <div class="row">
                    <div class="six columns">
                        ...
                        <select name="level" id="level">
                            <option value="1" <?php
                            if (get_the_user('level') == '1') {
                                echo 'selected';
                            }
                            ?>>1</option>
                            <option value="2" <?php
                            if (get_the_user('level') == '2') {
                                echo 'selected';
                            }
                            ?>>2</option>
                            <option value="3" <?php
                            if (get_the_user('level') == '3') {
                                echo 'selected';
                            }
                            ?>>3</option>
                            <option value="4" <?php
                            if (get_the_user('level') == '4') {
                                echo 'selected';
                            }
                            ?>>4</option>
                            <option value="5" <?php
                            if (get_the_user('level') == '5') {
                                echo 'selected';
                            }
                            ?>>5</option>
                        </select>
                    ...
                    <div class="six columns">
                        ...
                        <select name="status" id="status">
                            ...
                            if (get_the_user('status') == 'publish') {
                                echo 'selected';
                            }
                            ?>><?php lang('publish'); ?></option>
                            <option value="delete" <?php
                            if (get_the_user('status') == 'delete') {
                                echo 'selected';
                            }
